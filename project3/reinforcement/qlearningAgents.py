...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        maxQvalue = -maxint - 1
        for a in actions:
            newValue = self.getQValue(state, a)
            if newValue > maxQvalue:
                maxQvalue = newValue
                maxAction = a
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for featureName in features.keys():
            qValue += self.weights[featureName] * features[featureName]
    ...
    def update(self, state, action, nextState, reward):
        ...
        for featureName in features.keys():
            self.weights[featureName] += self.alpha * difference * features[featureName]
